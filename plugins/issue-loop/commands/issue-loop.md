---
name: issue-loop
description: Start or resume the issue-loop workflow (fix → commit → PR → wait for AI review → apply comments → repeat).
...
Run the issue-loop workflow using the plugin components in this plugin.
...
2. Read settings from `.claude/issue-loop.local.md` if present.
3. Invoke the loop agent `issue-loop-runner` to execute the full fix/review cycle.
